...
- https://github.com/fullstack-69/typescript-intro/blob/main/lecture/03_more_type_demo.ts
...
- `pnpm init`
  - Note: Unlike previous version of pnpm, `package.json` will have `"type": "module"` now.
...
- `pnpm exec tsc src/index.ts --outDir dist`

## Run JavaScript

- `node ./dist/index.js`

## Run TypeScript file directly (node 23+)
...
![bg contain right:50%](img/paste-1782448305641.png)

- New `devEngines` option in `package.json` will prevent the use of `npx`.
...
- Now just type `pnpm exec tsc`
- Use `pnpm exec tsc --showConfig` to see config.
...
You need to enable the following in your `tsconfig.json` to use NodeJS built-in modules:
...
- `pnpm install -D @types/node @tsconfig/node-lts @tsconfig/node-ts` _([What](https://github.com/tsconfig/bases#node-with-typescript-ts-58-only-tsconfigjson))_
...
- `pnpm create vite@latest`
  - Choose `React` and `TypeScript` options.
...
- `references` allows us to have multiple `tsconfig` files for different purposes.
  - `tsconfig.app.json` is for the React app
  - `tsconfig.node.json` is for NodeJS scripts.
...
const App = () => {
  return (
    <main>
      <Card title="Card 1" text="Text 1" />
      <Card title="Card 2" text="Text 2" />
      <Card title="Card 3" />
    </main>
  );
};
...
https://github.com/fullstack-69/typescript-intro/tree/typescript7
