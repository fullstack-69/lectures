...
# Hot-Reload: A Clarification

- Both MPA and HTMX use identical `nodemon` configs — restarts the Node process on file change
- The SPA's full HMR comes from **Vite**, not from being an SPA

> _"Server-rendered apps restart-and-manual-refresh; only the Vite-powered frontend gets instant HMR"_
...
- HTMX has **more code than MPA** — fragment templates and indicator wiring add lines that plain MPA never needs
- The bigger win shows up as apps grow: MPA/HTMX scale linearly with routes; SPA carries fixed framework overhead from the start
