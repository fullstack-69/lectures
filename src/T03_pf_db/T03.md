...
# Note on PostgreSQL 18

- Breaking change: the internal container volume mount path
  - `/var/lib/postgresql/data` -> `/var/lib/postgresql`
- Why?
  - Previously, mounting directly to the `data` folder made major database upgrades difficult.
  - In PostgreSQL 18+, the container automatically creates a version-specific subdirectory (like /var/lib/postgresql/18/docker).
  - This allows using `pg_upgrade --link` when migrating to future versions (like PostgreSQL 19), because multiple version directories can now live inside the same host volume mount.
