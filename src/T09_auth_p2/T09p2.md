...
- CMU ITSC is very strict about giving access to their OAuth 2.0 service.
  - Therefore, we will mock the OAuth 2.0 service with `Authentik`.
- [Authentik](https://goauthentik.io/) is an open-source, self-hosted identity provider.
  - Alternatives: [Keycloak](https://www.keycloak.org/).
...
- Go to https://oauth497.cpecmu.com/
  - Email: Your CMU email
  - Password: _(Announced in class)_

---

# CMU OAuth Information

![alt text](img/paste-1788146630268.png)

---

# Token

- You should receive `access_token` and `id_token` in the response.
- The `id_token` is a JWT token that contains user information.
  - This is the OIDC part of the flow.

![bg contain right:40%](img/paste-1788146893793.png)
...
- `git clone https://github.com/fullstack-69/auth-signin-oauth.git`
- Fill in `.env`
...
> Note: `app.use(passport.initialize());` is not necessary here because we are not using `session` in this example.
