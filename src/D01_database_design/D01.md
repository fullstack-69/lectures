...
- Easy to measure how data change over time (e.g., IoT, financial and logs)
...
# Standard for most small-to-medium setups

- Start with a **single database server**
  - Running multiple isolated databases
  - One database per application

![w:1000](./img/single-db.png)

---

# Standard for most small-to-medium setups

- Move to a **separated server** when:
  - Suffers from performance degradation
  - Compliance laws
  - The app scales large enough
...
- Blocks invalid insertion/update, prevents orphaned records

![w:650](./img/many-to-many2.png)
...
- Some `ORMs (Object-Relational Mappin)` can be used to defines a schema for **NoSQL**
- ORM helps validating data during software development
  - `Prisma ORM` defines a schema for `MongoDB`, providing type safety
  - `Drizzle` <u>does not natively support MongoDB</u>
...
- Stores data in unreadable **ciphertext** using **cryptographic algorithm**
- Ensure **data confidentiality** and complying with **privacy regulations**
  - GDPR, PDPA, HIPPA, and PCI DSS
- **Primary methods**
  - Transparent Data Encryption (TDE)
  - Column/Cell Encryption
...
- Encrypts database files at disk level (data at rest)
- Decryption is handled as data is read into memory.
- `Oracle TDE`, `Microsoft SQL Server TDE`, `PostgreSQL+pg_tde`
- **Data Encryption Key (DEK)** is a symmetric key
...
- Encrypts only specific, highly sensitive data fields
- Can impact database performance during queries
- `PostgreSQL + pgcrypto`
  - Password hashing (using **bcrypt**)
  - Symmetric/asymmetric **OpenPGP** encryption
  - Random data or **UUID** generation

---

# Applicaton-level Encryption

- Data is encrypted by the application before writing to the database
- Database never holds the **unencrypted data** or the **encryption key**
...
- [Database Design Tutorial](https://github.com/26x497-FSD-staff/tutorial-db-2569/blob/main/README-DB.md)
